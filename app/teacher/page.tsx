...
export default function TeacherPortal() {
  ...
  const students = [
    { id: 1, name: "Anjali", status: "Enrolled" },
    { id: 2, name: "Bobby", status: "Pending" },
    { id: 3, name: "Chandni", status: "Enrolled" },
  ];
  ...
  const handleCreateClassroom = async () => {
    ...
    if (self && self.userId) {
      ...
      if (yourClasses) setYourClasses([...yourClasses, res]);
      fetchClasses();
  ...
  async function handleFileUpload() {
    ...
    const response =  await fetch("http://localhost:8000/pdf", {
      ...
      body: JSON.stringify({ file: presigned.signedUrl.split('?')[0], classId: activeClass?.classId, chapterId: selectedChapter?.chapterName }),
    })
    const data = await response.json();
    toast.success(data.message);
    setFile(null);
    if (res.ok) {
      const content = await addContentToChapter(
            ...
            <TabsContent value="chapters">
              <Card>
                <CardContent className="pt-6">
                  ...
                  {chapters &&
                    chapters
                      .filter((c, i, v) => c.classId === activeClass?.classId)
                      .map((chapter) => (
                        <div
                        ...
                        >
                          ...
                          >
                            <p className="text-center text-muted-foreground">
                              Drag and drop PDF or PPT files here
                            </p>
                          </label>
                          <input
                        ...
                        >
                          ...
                          {isRecording ? "Stop Recording" : "Start Voice Capture"}
                            ...
                            <span onClick={() => setSelectedChapter(chapter)}>
                              <AudioRecorder
